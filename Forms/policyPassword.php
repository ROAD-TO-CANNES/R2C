...
<body>
  ...
  <form class="formPolicy" action="../Forms/policyPasswordScript.php" method="post">
    <policy>
      ...
      <label <?php if (isset($_GET['specialchar']) && $_GET['specialchar'] == "ok") {
                echo 'style="display:block"';
              } ?> for="specialchar">Nombre de caractères spéciaux modifier a <?= $specialchar ?> minimum</label>
      <button type="submit">Enregistrer</button>
    </policy>
    <policy>
      <h1>Nombre de Majuscules</h1>
      <input type="number" name="uppercase" value="<?= $uppercase ?>">
      <label <?php if (isset($_GET['uppercase']) && $_GET['uppercase'] == "ok") {
                echo 'style="display:block"';
              } ?> for="uppercase">Nombre de majuscules modifier a <?= $uppercase ?> minimum</label>
      <button type="submit">Enregistrer</button>
    </policy>
    <policy>
      <h1>Longueur</h1>
      <input type="number" name="size" value="<?= $size ?>">
      <label <?php if (isset($_GET['size']) && $_GET['size'] == "ok") {
                echo 'style="display:block"';
              } ?> for="size">Longueur minimum du mot de passe modifier a <?= $size ?> caractères</label>
      <button type="submit">Enregistrer</button>
    </policy>
    <policy>
      <h1>Nombre de Chiffres</h1>
      <input type="number" name="number" value="<?= $number ?>">
      <label <?php if (isset($_GET['number']) && $_GET['number'] == "ok") {
                echo 'style="display:block"';
              } ?> for="number">Nombre de chiffres modifier a <?= $number ?> minimum</label>
      <button type="submit">Enregistrer</button>
    </policy>
  </form>
</body>
